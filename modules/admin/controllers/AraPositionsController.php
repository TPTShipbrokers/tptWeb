...
use app\models\AraPositionSearch;
...
class AraPositionsController extends BasePositionsController
{
    ...
    {
        $searchModel = new AraPositionSearch();
        $queryParams = Yii::$app->request->queryParams;

        if ($id !== null) {
            $queryParams['AraPositionSearch']['positions_visible'] = $id;
        }
        ...
        if (Yii::$app->request->isPost && !empty($post)) {
            ...
            if ($model->upload()) {
                ...
                foreach ($data as $csv_values) {
                    ...
                    $model = AraPosition::find()->where(['name' => $vesselName])->one();
    ...
    public function actionDelete_all()
    {
        ...
        try {
            /** @var AraPosition[] $vessels */
            $vessels = AraPosition::find()->all();
    ...
    /**
     * Finds the AraPosition model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return AraPosition the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = AraPosition::findOne($id)) !== null) {
            return $model;
        } else {
